...
public class HuffmanEncodingTree<Comparable>
{
   private CustomChar[] pq;
   private int size = 0;
   ...
   {
       pq = new CustomChar[10];
       initialCapacity = 10;
   }


   // constructor: creates a new MaxPQ with a capacity of the parameter cap
   public HuffmanEncodingTree(int cap)
   {
       pq = new CustomChar[cap];
       initialCapacity = cap;
   }
    ...
    * Returns: void
    */
   public void insert(CustomChar item)
   {
    ...
    */
   public CustomChar delMax() throws EmptyQueueException
   {
       // best: O(logN), worst: O(N)--for resizing amortized: O(logN)
       if (isEmpty())
       {
    ...
    * Returns: Patient - the patient with the highest priority
    */
   public CustomChar getMax() throws EmptyQueueException
   {
       if (isEmpty())
       {
           throw new EmptyQueueException();
   ...
   {
       CustomChar[] tempPQ = new CustomChar[capacity];
   ...
   {
       int parent = parent(i);
       if(parent > 0 && pq[i].compareTo(pq[parent]) > 0){
           swap(i, parent);
           swim(parent);
       }
       if(parent == 0 && pq[i].compareTo(pq[parent]) > 0){
   ...
   private void sink(int i)
   {
       ...
       int largest = i;
       if(left < size && pq[left].compareTo(pq[i]) > 0) {
           largest = left;
       }
       if(right < size && pq[right].compareTo(pq[largest]) > 0){
           largest = right;
       }
       if(largest != i){
           swap(i, largest);
