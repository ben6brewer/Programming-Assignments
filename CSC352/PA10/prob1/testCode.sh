...
test_stdout() {
    file=$1
    {
    timeout 2 "$my_executable" < "$directory/$file" &> /dev/null
    } &> /dev/null
    ...
    stdout_status="NULL"
    "$my_executable" < "$directory/$file" > myOut 2> /dev/null
    "$example_executable" < "$directory/$file" > exOut 2> /dev/null
...
test_return_code() {
    ...
    $my_executable < "$directory/$file" &> /dev/null
    my_exit_code=$?
    $example_executable < "$directory/$file" &> /dev/null
    example_exit_code=$?
...
test_stderr() {
    file=$1
    stderr_status="NULL"
    $my_executable < "$directory/$file" 2> myErr 1> /dev/null
    $example_executable < "$directory/$file" 2> exErr 1> /dev/null
...
test_valgrind() {
    ...
    valgrind $my_executable < "$directory/$file" 2> myValgrindOutput 1> /dev/null
