...
my_executable="./shortestPaths"
example_executable="./exShortestPaths"
...
for ((i = 1; i <= 8; i++)); do
    ...
    input_file_graph="inFile$i"
    my_output="myOut$i"
    example_output="exOut$i"
    my_valgrind_output="myValOut$i"
    echo "TEST: $i"
    valgrind $my_executable $input_file_graph < $input_file > $my_valgrind_output
