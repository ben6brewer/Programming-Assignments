...
public class Prog1B {
    ...
    private static void searchCrater(RandomAccessFile binFile) throws IOException{
        ...
        while(true){
            ...
            if (craterIndex == -1) {
                ...
                System.out.println("crater Index:" + craterIndex);
                printCraterInfo(binFile, (8+ (craterIndex * recordLength)), 20, 14);
            }
        }
        scanner.close();
    }
    ...
    private static int binaryExponentialSearch(RandomAccessFile binFile, String targetName) throws IOException {
        ...
        for (int i = 0; i < numberOfRecords; i++) 
        {
            ...
            String currentCrater = readString(binFile, longestString1).trim();
    ...
    private static int binarySearch(RandomAccessFile binFile, int i, int numberOfRecords, String targetName) throws IOException {
        int fromIndex = (int) (2 * (Math.pow(2,i-1)-1)) + 1;
        // System.out.println("numberOfRecords: " + numberOfRecords);
        // System.out.println("i:" + i);
        int toIndex = (int) Math.min((2 * (Math.pow(2,i) - 1)) -1, numberOfRecords-1);
        ...
        while (fromIndex <= toIndex) {
            ...
            if (currentCrater.equalsIgnoreCase(targetName)) {
                return middleIndex;
            } else if (currentCrater.compareToIgnoreCase(targetName) < 0) {
    ...
    private static void printCraterInfo(RandomAccessFile binFile, int seekOffset, int longestString1, int longestString2) throws IOException {
        binFile.seek(0);
        int l1 = binFile.readInt();
        int l2 = binFile.readInt();
        binFile.seek(seekOffset);
        String name = readString(binFile, l1);
        ...
        String age = readString(binFile, l2).trim();
